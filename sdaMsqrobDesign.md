...
Duguet et al. 2017 compared the proteomes of mouse CD4+Foxp3+ regulatory T cells (Treg) and CD4+Foxp3- conventional T cells (Tconv) in order to build a data set of proteins differentially regulated in these two cell populations. For each biological repeat the proteomes were extracted for both Treg and Tconv cell pools that were purified by flow cytometry. The data in data/quantification/mouseTcell on the pdaData repository are a subset of the data [PXD004436](https://www.ebi.ac.uk/pride/archive/projects/PXD004436) on PRIDE.
