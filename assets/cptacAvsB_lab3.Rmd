...
55% of all peptide intensities are missing and for some peptides we don't even measure a signal in any sample. The missingness is similar across samples.
```{r, cache= TRUE}
plotNA(pepData)
naplot(pepData)
```
...
We will first replace the expression values that are equal to zero by a NA value.
Otherwise we will get values that are - infinity upon log transformation
...
#### 2.2.1. Handling overlapping protein groups
...
#### 2.2.2. Remove reverse sequences (decoys) and contaminants
...
#### 2.2.3. Remove peptides of proteins that were only identified with modified peptides
...
proteins <- pepData %>% fData %>% select("Proteins") %>% as.character
...
#### 2.2.4 Drop peptides that were only identified in one sample
...
### 2.3. Quantile normalize the data
```{r}
pepData <- normalise(pepData, "quantiles")
```


### 2.4. Explore quantile normalized data
...
```{r}
protData<-suppressWarnings(combineFeatures(pepData,groupBy=fData(pepData)$Proteins,fun="robust"))
```
...
#### 3.3.2. Detail plot for the top 5 proteins.
